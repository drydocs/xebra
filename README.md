...
- `apps/arbiter-service` — KMS-backed `resolve()` submitter for both escrows, wired end-to-end for **both** corridors: `lookupClaim` (the join this file's own doc comment named as blocking it) against the now-populated `claims` table, `verifyClaimAgainstDestinationChain` dispatching to either a real fetched-and-parsed Solana tx (`@xebra/chain-adapters`' `verifyDelivery`) or a real fetched Horizon payment (`verify-stellar-fulfillment.ts`, new), then `decideClaimValidity` and the real `resolveOnArc`/`resolveOnSoroban` submission. The Stellar-side asset check doesn't need a reverse lookup from the intent's one-way `sha256(assetCode||issuer)` hash — it hashes the *actual* payment's asset the same way and compares hashes, sidestepping a schema gap that would otherwise require a new asset registry. A `arbiter_pending_challenges` gauge tracks challenges that couldn't be resolved yet (missing claim row or unverifiable destination).
